...
class Inreports_Model extends Model
{
    ...
    {
        $sth=$this->db->prepare("SELECT * FROM tbl_subfirm WHERE parentid=pid AND subid=:sid");
        $sth->execute(array(
    ...
    public function printdelivery($data)
    {
    ...
    {
        $sth=$this->db->prepare("SELECT * FROM tbl_deliveryhead WHERE parentid=:pid AND subid=:sid AND period=:period");
        $sth->execute(array(
           ':pid'=>Session::get('parentcompanyid'),
            ':sid'=>Session::get('subsidiaryid'),
