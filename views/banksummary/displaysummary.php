...
session::init();
foreach ($this->displaysummary as $key => $value) {
	# code...
	Session::set('month',$value['nmonth']);
	Session::set('year',$value['nyear']);
}

session::set('myrecord',$this->displaysummary);
...
require "libss/fpdf.php";
	...
	class myPDF extends FPDF{
		...
		function header(){
			...
			$this->Cell(150,12,'BANK SUMMARY REPORT: '.Session::get('month'). ' '.Session::get('year') ,0,0,'C');
			$this->Ln(15);
		...
		function headerTables(){
			...
			$this->Cell(18,5,'No.of Staff',1,0,'C');
			$this->Cell(120,5,'Bank Details',1,0,'C');
			$this->Cell(18,5,'Amount',1,0,'C');								
			$this->Ln();
		}
		...
		function viewTable(){
			...
			$this->Cell(18,5,$row['noofstaff'],1,0,'L');
			$this->Cell(120,5,$row['bank'],1,0,'L');
			$this->Cell(18,5,number_format($row['amount']),1,0,'R');			
			$this->Ln();
		...
		function footable(){
			...
			$this->Cell(122, 5, '' ,0,0);
			$this->Cell(25, 5,'Net Pay',0,0);
			$this->Cell(4, 5,'=N=',1,0);
			$this->Cell(25, 5,number_format($grandtotal),1,1,'R');
