                ...
                <div class="form-inline">
                    <div class="validation-system">
                        ...
                        <div  id="snackbar" class="alert alert-danger" role="alert" ></div>
                            ...
                            <div  class="alert alert-danger" role="alert" style="text-align: center;" >ROLE AND MENU SELECTOR</div>
                                ...
                                <input type="hidden" id="currentuser" value="<?php echo Session::get('CurrentUser'); ?>">

                                <select name="approle" id="approle" onchange="getSavedMenu(this.value)">
                                            <option value disabled selected>Select App Role</option>                                    <?php
                                            foreach($this->GetAppRoleList as $approle) {
                                                ?>
                                                <option value="<?php echo $approle["id"]; ?>"><?php echo $approle["rolename"]; ?></option>
                                                <?php
                                            }
                                            ?>
                                        </select>
                                        <script>
                                function getSavedMenu(val) {
                                    alert("ok");
                                  // return  console.log('selected', val);
                                    var roleid = document.getElementById("approle").value;
                                    $("#tbodyid").empty();
                                    //alert(parentId);
                                    $.ajax({
                                        type: 'post',
                                        url: 'http://localhost:8080/finance/account/models/load_menurole.php',
                                        data: {roleid: roleid},
                                        success: function (response) {
                                            var data = JSON.parse(response);
                                                            ...
                                                            for(var i=0; 0 < data.length; i++)
                                                            {
                                                                ...
                                                                var createrow = "<tr><td>" + data[i].id + "</td><td>" + data[i].parentmenudesc + "</td><td>" + data[i].submenudesc + "</td></tr>";
                                ...
                                <select name="menuhead" id="menuhead" onchange="getSubMenuInfo(this.value)">
                                            <option value disabled selected>Select App Menu Head</option>                                    <?php
                                            foreach($this->GetMenuHead as $menuhead) {
                                                ?>
                                                <option value="<?php echo $menuhead["mnuMenu"]; ?>"><?php echo $menuhead["mainmenu"]; ?></option>
                                                <?php
                                            }
                                            ?>
                                        </select>
                                        ...
                                        <script>
                                function getSubMenuInfo(val) {
                                  // return  console.log('selected', val);
                                    var parentmenu = document.getElementById("menuhead").value;
                                    $('#snackbar').val('');
                                    $('#snackbar').hide();
                                    //alert(parentId);
                                    $.ajax({
                                        type: 'post',
                                        url: 'http://localhost:8080/finance/account/models/load_submenulist.php',
                                        data: {parentmenu: parentmenu},
                                        success: function (response) {
                                            var data = JSON.parse(response);
                                            
                                           $( '#dmodule-list' ).html(data.courses);
                                        }
                                    });
                                }
                            </script>
                                ...
                                <div class="clearfix"> </div>                                    
                                <select name="dmodule" id="dmodule-list" size='6' class="form-control1" onchange="getstatusclear(this.value)">
                                    <option value="">Select Sub Menu</option>
                                </select>
                                        <script>
                                function getstatusclear(val) {
                                  // return  console.log('selected', val);
                                ...
                                <button id="Addmenu" class="btn btn-default btn-send">Add/Create User Menu</button>
                            </div>
...
<div>
                <table class="blueTable">
                    ...
                    <tr>
                        <?php
                        //<td><img src="<?php echo URL.'public/images/edit.png'; 
                        // style="width: 6%; height: auto;"> </td><td>Store Manager</td><td>Controlling total affairs of store including stock balance</td></tr>
                        ?>
